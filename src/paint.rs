...
pub fn create_free_circles(rng: &mut StdRng, cfg: &SceneCfg) -> Vec<Disc> {
...
pub fn create_free_triangles(rng: &mut StdRng, cfg: &SceneCfg) -> Vec<Triangle> {
    let mut items = Vec::new();
    for i in 1..=cfg.nb_pattern {
        let c = cfg.choose_color(rng);
        items.push(Disc::random(
...
pub fn create_free_stripes(rng: &mut StdRng, cfg: &SceneCfg) -> Vec<Stripe> {
    let mut items = Vec::new();
...
pub fn create_free_spirals(rng: &mut StdRng, cfg: &SceneCfg) -> Vec<Spiral> {
    let mut items = Vec::new();
    for _ in 0..cfg.nb_pattern {
        let c = cfg.choose_color(rng);
        let w = cfg.width_pattern * cfg.frame.h as f64 * (rng.gen::<f64>() + 0.5);
...
pub fn create_concentric_circles(rng: &mut StdRng, cfg: &SceneCfg) -> Vec<Disc> {
    let mut items = Vec::new();
...
pub fn create_parallel_stripes(rng: &mut StdRng, cfg: &SceneCfg) -> Vec<HalfPlane> {
    let mut items = Vec::new();
    let (a, b, dir) = {
...
pub fn create_crossed_stripes(rng: &mut StdRng, cfg: &SceneCfg) -> Vec<HalfPlane> {
    let mut items = Vec::new();
...
pub fn create_waves(rng: &mut StdRng, cfg: &SceneCfg) -> Vec<Wave> {
    let mut items = Vec::new();
    let (a, b, dir) = {
        let c = cfg.frame.center();
        let w = cfg.frame.h + cfg.frame.w;
...
pub fn create_sawteeth(rng: &mut StdRng, cfg: &SceneCfg) -> Vec<Sawtooth> {
    let mut items = Vec::new();
    let (a, b, dir) = {
        let c = cfg.frame.center();
        let w = cfg.frame.h + cfg.frame.w;
